...
public class CartItemPriceGrossValueAdapter implements VirtualAttributeAdapter<DecimalAttribute> {
	...
	public boolean supports(DecimalAttribute attribute, NamedAttributeHolder holder) {
		return SchemaInspectionUtil.isAssignableTo(holder, CartItem.class.getSimpleName()) && attribute.getName().equals("taxedItemPriceGross");
	}

	@Override
	public Object read(DecimalAttribute attribute, Record r) {
		CartItem ci = schemaBeanFactory.getSchemaBean(CartItem.class, r);
		return ci.getPriceGrossNow();
	}
