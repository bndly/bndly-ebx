...
public class PurchaseOrderTotalValueAdapter implements VirtualAttributeAdapter<DecimalAttribute> {
	...
	public Object read(DecimalAttribute attribute, Record purchaseOrderRecord) {
		...
		BigDecimal merchandiseValue = order.getMerchandiseValue();
		if (merchandiseValue != null) {
			total = total.add(merchandiseValue);
		}
		if (order.getShipmentOffer() != null) {
			BigDecimal shipmentPrice = order.getShipmentOffer().getPrice();
			if (shipmentPrice != null) {
				total = total.add(shipmentPrice);
			}
		}
		purchaseOrderRecord.setAttributeValue(attribute.getName(), total);
		return total;
	}
