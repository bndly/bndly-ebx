...
public class LineItemTotalValueAdapter implements VirtualAttributeAdapter<DecimalAttribute> {
	...
	public boolean supports(DecimalAttribute attribute, NamedAttributeHolder holder) {
		if (!"total".equals(attribute.getName())) {
			return false;
		}
		return SchemaInspectionUtil.isAssignableTo(holder, LineItem.class.getSimpleName());
	}
	...
	public Object read(DecimalAttribute attribute, Record r) {
		...
		LineItem li = schemaBeanFactory.getSchemaBean(LineItem.class, r);
		BigDecimal total = null;
		if (li.getQuantity() != null && li.getPrice() != null) {
			total = li.getPrice().multiply(new BigDecimal(li.getQuantity()));
		}
		r.setAttributeValue(attribute.getName(), total);
		return total;
	}
