...
public class GeizhalsCollector extends AbstractExportEntityCollector<GeizhalsEntity> {
	...
	@Override
	protected GeizhalsEntity mapToEntity(Purchasable purchasable, Currency desiredCurrency, User defaultUser) {
		ExportMapper<GeizhalsEntity> exportMapper = (ExportMapper<GeizhalsEntity>) getExportProvider().getExportMapperByEntity(modelType);
		...
		GeizhalsEntity mapped = exportMapper.toModel(purchasable, pr.getPrice());
		mapped.setAvailability("sofort");
		mapped.setDeliveryTime("3-5 Tage");
		return mapped;
	}
